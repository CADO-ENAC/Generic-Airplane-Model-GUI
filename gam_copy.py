...
class GAM(object):
    """The Generic Airplane Model is a very simple way to size and fly airplanes of different categories and propulsive technologies
    The user can define freely the capacity, range and propulsive technology among available ones and fly the resulting aircraft
    over give routes
    """
    ...
    def __init__(self):
        ...
        self.ref_power_factors = [8.41043794e-05, 2.02675942e+02, -9.5e4]  # [a, b, c], ref_power = a * mtow**2 + b * mtow + c
        ...
        self.cons_factor = 1.       # Propulsion system efficiency factor
        ...
        self.psfc_turboshaft_coef = [0.17, 12, 0.63]  # kg/kW/h = a + b / (kW)**c
        # self.psfc_turboshaft_coef = [0.20, 10, 0.65]  # kg/kW/h = a + b / (kW)**c
        ...
        # The scale factor accounts for efficiency decrease when turbomachines get smaller
        self.eff_th_turbofan = 0.474    # 0.474
        self.fuel_mixture = 0.020       # Fuel to air ratio, 0.02
        self.turbofan_scale_coef = [0.2, 5.e6, 2]  # eff_scale_factor = 1 - (1-a) / (1+(max_power/b))**c
        ...
        self.standard_af_mwe_factors = [-3.06255540e-07, 4.18303322e-01, -40]  # [a, b, c]   # With new total_power
        ...
        self.tech_data = {# ref M.Marksel et A.Prapotnik Brdnik, « Comparative Analysis  of
                          ...
                          "emotor_power_price": 94 * euro_dollar / unit.convert_from("kW", 1),  # 94 euro/kW
                          "fuel_cell_power_price": 40 * euro_dollar / unit.convert_from("kW", 1),  # 40 euro/kW
                          ...
                          "battery_capacity_price": 300 * euro_dollar / unit.convert_from("Wh", 1),  # 300 euro/Wh
                          ...
                          "gh2_tank_mass_price": 300 * euro_dollar,       # 300 euro/kg
                          "lh2_tank_mass_price": 245 * euro_dollar}       # 245 euro/kg
    ...
    def e_price_per_kg(self, fuel_type, uref, v):
        ku = unit.convert_from(uref, 1)
        fhv = phd.fuel_heat(fuel_type)
        return v * (fhv / ku)

    def e_price_per_J(self, fuel_type, uref, v):
        ku = unit.convert_from(uref, 1)
        fhv = phd.fuel_heat(fuel_type)
        return v / (fhv * ku)
    ...
    def print_model_data(self):
        ...
        print("")
        print("   gam.cons_factor = ", "%.3f" % self.cons_factor, " Propulsion system efficiency factor")
        print("")
        ...
        print("")
        print("The scale factor accounts for efficiency decrease when turbomachines get smaller")
        print("gam.turbofan_scale_coef = ", self.turbofan_scale_coef, " eff_scale_factor = 1 - (1-a) / (1+(max_power/b))**c")
        print("")
    ...
    def get_piston_eff(self):
        ...
        """
        eff = (self.prop_eff * self.cons_factor) / (self.fhv_piston * self.psfc_piston)
        return eff
    ...
    def get_turboprop_eff(self, max_power):
        ...
        eff = (self.prop_eff * self.cons_factor) / (self.fhv_turboshaft * psfc_ref)
        return eff
    ...
    def get_scale_factor(self, max_power):
        """The scale factor is modelling the fact that big engines are more efficient than small ones
           due to the increasing importance of leakages when engine size decreases
        """
        a, b, c = self.turbofan_scale_coef
        scale_factor = 1 - (1-a) / (1+(max_power/b))**c
        return scale_factor
    ...
    def get_turbofan_eff(self, tas, bpr, max_power, energy_type):
        ...
        eff = eff_th * eff_pr * k_eff * self.cons_factor
        return eff

    def get_turbofan_sfc(self, tas, bpr, max_power, energy_type):
        """Return turbofan engine TSFC (kg/s/N)
        """
        eff = self.get_turbofan_eff(tas, bpr, max_power, energy_type)
        fhv = phd.fuel_heat(energy_type)
        sfc = tas / (eff * fhv)
        return sfc, fhv
    ...
    def get_emotor_eff(self, energy_type, thruster_type):
        ...
        if energy_type==self.battery:
            eff = thruster_eff * self.emotor_eff * self.cons_factor
        elif energy_type in [self.gh2, self.lh2]:
            eff = thruster_eff * self.emotor_eff * self.fuel_cell_eff * self.cons_factor
        else:
    ...
    def leg_fuel(self, start_mass, distance, altp, speed, speed_type, mtow, max_power, power_system):
        ...
        if power_system["engine_type"] == self.piston:
            ...
            eff = self.get_turbofan_eff(tas, power_system["bpr"], max_power, power_system["energy_type"])
    ...
    def tune_design(self, power_system, mission, lod_sfc_sharing=[1., 0.]):
        ...
        """
        if min(lod_sfc_sharing)<0 or sum(lod_sfc_sharing) != 1.:
            raise Exception("Factors in lod_sfc_sharing must be positive and their sum equal to 1")
        ...
        def fct1(x):
            self.lod_factor = 1. + lod_sfc_sharing[0] * x[0]
            self.cons_factor = 1. + lod_sfc_sharing[1] * x[0]
        ...
        def fct2(x):
            self.lod_factor = 1. + lod_sfc_sharing[0] * x
            self.cons_factor = 1. + lod_sfc_sharing[1] * x
            total_power = self.ref_power(mtow_target)
            dict_p = self.owe_performance(payload, mtow_target, design_range, cruise_speed, total_power, power_system, altitude_data, reserve_data)
            dict_s = self.owe_structure(category, npax, mtow_target, design_range, total_power, dict_p["max_fuel"], dict_p["max_energy"], power_system)
        ...
        if power_system["energy_type"] != self.battery:
            x_ini = [0., ac["stdm_factor"]]
            output_dict = fsolve(fct1, x0=x_ini, args=(), full_output=True)
            if (output_dict[2] != 1): raise Exception("Convergence problem")
            self.lod_factor = 1. + lod_sfc_sharing[0] * output_dict[0][0]
            self.cons_factor = 1. + lod_sfc_sharing[1] * output_dict[0][0]
    ...
    def operating_cost(self, ac_dict, traffic_zone="west_bound"):
        ...
        yearly_energy_consumption = this_dict["mission_enrg"] * flight_cycle_count
        yearly_fuel_consumption = this_dict["mission_fuel"] * flight_cycle_count
        yearly_co2_production = phd.co2_index(energy_type) * yearly_fuel_consumption
        ...
        cost_breakdown = {"annuity_factor": annuity_factor,
                          ...
                          "yearly_fuel_consumption": yearly_fuel_consumption,
                          "yearly_co2_production": yearly_co2_production}
    ...
    def print_operating_cost(self, this_dict):
        ...
        print("")
        print("Yearly energy consumption = ", "%.3f" % (this_dict["yearly_energy_consumption"]*1.e-12), " TWh")
        print("  Yearly fuel consumption = ", "%.0f" % (this_dict["yearly_fuel_consumption"]*1.e-3), " T")
        print("    Yearly CO2 production = ", "%.0f" % (this_dict["yearly_co2_production"]*1.e-3), " T")
    ...
    def print_design(self, this_dict, name=None):
        ...
        st.write("  Max power = ", "%.3f" % unit.kW_W(this_dict["max_power"]), " kW")
        st.write("")
    ...
    def fly_tow_n_distance(self, ac_dict, tow, distance):
        ...
        if power_system["energy_type"] != self.battery:
            this_dict["fuel_consumption"] = (this_dict["mission_fuel"] / phd.fuel_density(power_system["energy_type"])) / (npax * distance)
        else:
            this_dict["fuel_consumption"] = 0
    ...
    def print_mission(self, this_dict, title=""):
        ...
        print("Revenue passenger = ", "%.0f" % this_dict["npax"])
        print("")
        ...
        print("")
        print(" Fuel consumption = ", "%.2f" % unit.convert_to("L", this_dict["fuel_consumption"] * unit.m_km(100)), " L/pax/100km")
        print("")
...
if __name__ == '__main__':
    ...
    design_mission = {"category": "short_medium", "npax": 150, "speed": 0.78, "range": unit.convert_from("km", 5500), "altitude": unit.convert_from("ft", 35000)}
    ...
    print("Calibrated propulsion efficiency factor = ", gam.cons_factor)
